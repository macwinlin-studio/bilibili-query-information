...
class MyFrame(wx.Frame):
    ...
    def on_click(self,event):
        ...
        headers = {'Users-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.114 Safari/537.36 Edg/89.0.774.75'}
        try:
            ...
            sc = 200
        if self.tc.GetValue() == 0:
            sc = 'error'
        if sc == 200:
            self.error.SetLabelText('')
            xx = requests.get(api,headers=headers)
            xx_t = xx.text
            xx_tl = []
